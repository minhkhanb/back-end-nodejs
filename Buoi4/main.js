...
const BTVN = {
  ...
  bai2: function () {
    ...
    // getTotalGold(sports)
    // const getTotalGold = (arr) => arr.reduce((total, sport) => total + sport.gold, 0);
    Array.prototype.getTotalGold = function () {
      return this.reduce((accumulator, currentSport) => accumulator + currentSport.gold, 0);
    };

    // const output = getTotalGold(sports);
    const output = sports.getTotalGold();
  ...
  bai3: function () {
    ...
    const swim = 'Bơi lội';
    const getMostFavoriteSport = (arr) => structuredClone(arr).filter(sport => !!!sport.name.match(new RegExp(swim, 'gi')));
  ...
  bai4: function () {
    ...
    Array.prototype.sum = function () {
      return this.reduce((total, currentFilm) => total + parseFloat(currentFilm.imdbRating), 0);
    };

    Array.prototype.average = function () {
      const totalImdbRating = this.sum();

      return totalImdbRating / this.length;
    };

    const calculateAverageOfImdbRating = (arr) => {
      const filmOfChristopherNolan = arr.filter(({Director}) => Director === 'Christopher Nolan');

      return filmOfChristopherNolan.average();
    };

    console.log(calculateAverageOfImdbRating(watchList)); // Output: 8.675
  },
  bai5: function () {
    // --------------------IF/ELSE------------------------------
  ...
  bai6: function () {
    ...
    const getCanVoteMessage = (x) =>
      x >= 18 ? 'Bạn có thể bỏ phiếu' : 'Bạn chưa được bỏ phiếu';
  ...
  run: function () {
    this.runAt(6, this.version, LogLevel.Teacher);
  }
};
