...
const routerName = 'groups'

const groupsSchema = require(__path_schema + routerName)
const groupsModels = require(__path_models + routerName)
...
router.post('/save',
  ...
  async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      groupsModels.saveItems(item,{task: taskCurrent}).then((result)=>{
        if (taskCurrent == 'add') {
          req.flash('success', message , linkIndex)
        } else if (taskCurrent == 'edit') usersModels.saveItems(item, {task: 'change-group-name' }).then((result)=>{
