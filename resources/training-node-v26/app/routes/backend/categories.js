...
const categoriesSchema = require(__path_schema + routerName)
const categoriesModels = require(__path_models + routerName)
...
const pageTitle = 'Category Manager '
...
router.get('(/status/:status)?', async (req, res, next) => {
  ...
  let menusItems = []
  await categoriesModels.listItemsInSelecbox().then(items=>{
    menusItems = items;
    menusItems.unshift({id: 'allvalue', slug: 'Choose Menu'})
  })
  await categoriesModels.countItems(params).then((data) => {
    params.pagination.totalItems = data
  })
  categoriesModels.listItems(params)
    .then((items) => {
...
router.get('/change-status/:id/:status', function (req, res, next) {
  ...
  categoriesModels.changeStatus(currentStatus, id, { task: 'changeStatus-one' }).then((result) => {
...
router.get('/delete/:id', function (req, res, next) {
  let id = paramHelper.getParam(req.params, 'id', '');
  categoriesModels.deleteItems(id, { task: 'delete-one' }).then((result) => {
...
router.post('/change-status/:status', async (req, res, next) => {
  let currentStatus = paramHelper.getParam(req.params, 'status', 'active');
  categoriesModels.changeStatus(currentStatus, req.body.cid, { task: 'changeStatus-multi' }).then((result) => {
...
router.post('/delete', async (req, res, next) => {
  categoriesModels.deleteItems(req.body.cid, { task: 'delete-multi' }).then((result) => {
...
router.post('/change-ordering', async (req, res, next) => {
  ...
  categoriesModels.changeOrdering(cids, oderings).then((result) => {
...
router.get('/form(/:id)?', async function (req, res, next) {
  let id = paramHelper.getParam(req.params, 'id', '');
  let item = { name: '', ordering: '', status: '', menus_id :'', menus_name : '' }
  let showError = null
  let menusItems = []
  await categoriesModels.listItemsInSelecbox().then(items=>{
    menusItems = items;
  ...
  if (id === '') {
    res.render(`${folderView}form`, { pageTitle: pageTitleAdd, item: item, showError, menusItems })
  } else {

    await categoriesModels.getItems(id).then((item) => {
      item.menus_id = item.menus.id
      item.menus_name = item.menus.slug
      res.render(`${folderView}form`, { pageTitle: pageTitleEdit, item: item, showError, menusItems })
    })
  }
});
...
router.post('/save',
  body('name')
    ...
    .custom(async (value, { req }) => {
      return await categoriesSchema.findOne({ name: value }).then(async (user) => {
        if (user) {
  ...
  async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      res.render(`${folderView}form`, {pageTitle, item, showError: errors.errors,menusItems:menusItems})
    } else {

      let message = (taskCurrent == 'add') ? notifyConfig.ADD_SUCCESS : notifyConfig.EDIT_SUCCESS
      categoriesModels.saveItems(item, { task: taskCurrent }).then((result) => {
