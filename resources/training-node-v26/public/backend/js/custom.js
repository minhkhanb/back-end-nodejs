...
// fillter
    $('select[name="fillter-group"]').change(function (value) {
        // lấy các thành phần của url
        let path = window.location.pathname.split('/')
        // tạo link mới
        let linkRedirect = '/'+ path[1] + '/' +  path[2] + '/fillter-group/'+ $(this).val()
        // lưu vào vị trí
...
$('input#change-slug').keyup(function(){
    $('input[name="slug"]').val(ChangeToSlug($(this).val()))
})

// category
$('select[name="menus_id"]').change(function (value) {
    $('input[name="menus_name"]').val($(this).find(":selected").text());
});
